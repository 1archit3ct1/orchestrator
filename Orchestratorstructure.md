...
```
orchestrator/
├── SPAWN/                          # LLM spawn point (forced location)
│   ├── SPAWN/START/                      # Bootstrap zone (immutable after lock)
│   │   └── bootstrap.sh            # Linux/WSL bootstrap script
│   └── SPAWN/STOP/                       # Runtime zone
│       ├── .orchestrator/
│       │   ├── config.json
│       │   ├── task_queue.json
│       │   ├── security_manager.py # Enterprise security
│       │   ├── iteration_logger.py # LLM iteration logging
│       │   ├── vector_store/
│       │   ├── logs/
│       │   ├── data/
│       │   ├── models/
...
│       └── MEMORY.md               # Append-only execution memory
├── .vscode/
├── .wsl/
├── .wslconfig
├── HOWTOPROMPT.md
...
**Purpose:** All LLM interactions MUST originate from within `SPAWN/`. This ensures:
...
```
SPAWN/
├── SPAWN/START/         # Bootstrap (run once, then immutable)
└── SPAWN/STOP/          # Runtime (where all work happens)
```

**Flow:** `Clone → SPAWN/SPAWN/START/bootstrap.sh → [Initialized] → SPAWN/SPAWN/STOP/ [Work Here]`

---

## Bootstrap Process (Current State)

**FLOW:** `Clone → SPAWN/START/bootstrap.sh → [Orchestrator Initialized] → SPAWN/STOP/ [Orchestrator Coordinates]`

After the structural bootstrap completes (01-init through 06-gpu-setup), the following **operational initialization** must occur before the autonomous loop can function:
...
| 4 | **Derive tasks from DAG** | `SPAWN/STOP/state/tasks.json` | Convert design_graph.json into executable tasks |
...
### Phase 3: Autonomous Loop (Ready After Phase 2)
